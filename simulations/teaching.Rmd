  ...
  if(delay > MAX_DELAY) {
    ...
    if(k == 0) {
      ...
      teach_util <- teach_u(P, S) + 
        discount(delay + 1, lambda) * 
        (act_u(delay + 1, teach_outcome(1, p), n = 1, p, P, S, 
               alpha, lambda) %>% pull(eu))
...
combos <- expand.grid(p = seq(.1, 1, .1), P = c(70), 
                      S = seq(10,50,10), alpha = seq(.5, 3, .5),
                      lambda = seq(.1, 1, .2)) %>%
  filter(P > S, (P + S) <= 100) %>%
  as_tibble() 


com_props3 <- map_df(1:nrow(combos), 
...
write_csv(join_com_props, here("cached_data/com_props.csv"))
...
```{r pbv_cache}
pbv_combos <- expand.grid(p = seq(.1, 1, .1), C = c(1, 2, 4, 8), 
                          nguesses = c(1, 2, 3, 4), 
                          M = c(16, 32, 64, 128)) %>%
  filter(nguesses <= C) %>%
  as_tibble()
